...
def transparent_heatmap(ax,im,heatmap,cmap='viridis'):
    mycmap = transparent_cmap(cmap=plt.get_cmap(cmap))
    heatmap = image_histogram_equalization(heatmap, number_bins=256) # equalize the heatmap
    w, h = im.size
    y, x = np.mgrid[0:h, 0:w]
    ax.imshow(im)
    cb = ax.contourf(x, y, heatmap.reshape(x.shape[0], y.shape[1]), 15, cmap=mycmap)
    
def spotlight_custom(im, heatmap, levels=3, most_salient_nlevel=3, mask_darkness=0.8, 
                     smoothness=3, brightness = 0.8, percentile_based = True, toplot=True):
    """
    ...
    #print('heatmap values from %2.2f to %2.2f'%(np.min(heatmap),np.max(heatmap)))
    ...
    if (percentile_based):
        ...
        perc_list = np.array([np.percentile(h,i) for i in range(start_percentile,101)])
        val_list = perc_list
    else:
        # uniformly split 255
        div, rem = int(255/total_levels), 255%total_levels
        val_list=[val+i if i<rem else val+rem for i, val in enumerate(range(0,256,div))]
    nvals = len(val_list)
    ...
    for j in range(most_salient_nlevel):
        smaller_idx, larger_idx = i*group_size+j*first_group_size, i*group_size+(j+1)*first_group_size
    ...
    new_im = (im*np.repeat(darkening[...,None],3,axis=-1)*brightness).astype('uint8')
